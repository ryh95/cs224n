...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        input_tensor = tf.nn.embedding_lookup(embed_tensor, self.input_placeholder)

        n_features = self.config.n_features
        embedding_size = self.config.embed_size
        embeddings = tf.reshape(input_tensor, shape=(-1,self.max_length,n_features * embedding_size))
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.Variable(initial_value=tf.zeros(shape=(1,self.config.n_classes)))
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h = cell(x[:, time_step, :], h, scope=tf.get_variable_scope())
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.boolean_mask(batch_loss,mask=self.mask_placeholder))
